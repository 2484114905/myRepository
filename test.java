...
public class test {
    ...
    public static void distribute(ArrayList<Fruit> fruits, int people) throws NoSuchFieldException, CloneNotSupportedException, IOException, ClassNotFoundException {
         if(fruits.size() < people){
             ...
             ArrayList<Fruit> tempFruits = clone(fruits);
             Collections.sort(tempFruits, new FruitComparator());
             while (tempFruits.size() > people){
                 tempFruits.remove(0);
             }
             distribute2(tempFruits,fruits, people);
         }
    }
...
//            System.out.println("number" + fruits.get(tempFruits1.get(tempFruits1.size()-1).getId()).getNumber());
            for (Fruit i:splitedFruit) {
                System.out.println("splited--"+i.getNumber());
            }
            for (Fruit i:tempFruits1) {
            ...
            for (Fruit i:tempFruits1) {
                System.out.println("merged--"+i.getNumber()+ "--" + i.getId());
            }
//              System.out.println("ok" + tempFruits1.get(2).getNumber());
        ...
        if (fruits.get(id).getNumber()/ (fruits.get(id).getSplitCount()+1) > tempFruits1.get(0).getNumber()){
            distribute2(tempFruits1, fruits, people);
        }
        else {
    ...
    public static void distribute2(ArrayList<Fruit> currentFruits, ArrayList<Fruit> primaryFruits, int people){
        int id = currentFruits.get(currentFruits.size()-1).getId();//获取水果种类再进行拆分
        while (primaryFruits.get(id).getNumber()/ (primaryFruits.get(id).getSplitCount()+1) > currentFruits.get(0).getNumber()){
            ...
            id = currentFruits.get(currentFruits.size()-1).getId();
        }
