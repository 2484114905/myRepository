...
public class test {
    ...
    public static void distribute1(ArrayList<Fruit> fruits, int people) throws CloneNotSupportedException, IOException, ClassNotFoundException {
        ...
        while (kind < people){
            ArrayList<Fruit> tempFruits2 = clone(tempFruits1);
            ArrayList<Fruit> splitedFruit = fruits.get(tempFruits1.get(tempFruits1.size()-1).getId()).split();
//            System.out.println("number" + fruits.get(tempFruits1.get(tempFruits1.size()-1).getId()).getNumber());
            for (Fruit i:splitedFruit) {
                System.out.println("-++-"+i.getNumber());
            }
            tempFruits2.remove(tempFruits2.size()-1);
            tempFruits2.addAll(splitedFruit);
            Collections.sort(tempFruits2, new FruitComparator());
            if (tempFruits2.size() == people && (splitedFruit.get(0).getNumber() >= tempFruits2.get(0).getNumber()*2 + 2)){
                    distribute3(tempFruits2, fruits, people);
                    return;
            }
            else if(tempFruits2.size() < people && splitedFruit.get(0).getNumber() >= tempFruits2.get(tempFruits2.size()-1).getNumber()){
                System.out.println("continue");
                continue;

            }
            else {
                tempFruits1.remove(tempFruits1.size()-1);
